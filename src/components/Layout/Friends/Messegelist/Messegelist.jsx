...
const Messegelist = () => {
   ...
   const handleNotification = (friend) => {
    notiFication.forEach((item) => {        
        if (
            (item.whosenderid == friend.senderid &&
                item.whoreceiverid == friend.receiverid) ||
                (item.whosenderid== friend.receiverid &&
                    item.whoreceiverid == friend.senderid)
                ) {   remove(ref(db, 'chatNotificaion/'+ item.id))
                }
            });
            console.log(friend,"sdfshfifiu");
  };
            ...
            <div className=" overflow-auto h-[80%] ">
                {
                    messegeList.map((item) => (
                        <div className="flex items-center justify-between pt-[18px] pb-[13px] border-b-2 border-gray-300 ">
                            ...
                            }} className='font-semibold font-secondary text-[14px] rounded-[7px] text-white bg-[#0D6EFF] py-[13px] px-[17px] '> msg</button>
